...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void numbers(){
    ...
    numOfDon =Tasks.where((e)=> e.status == StatusTask.done).toList().length;
    ...
    return Row(
      ...
      children: [
         Container(
              ...
              child: const Icon(Icons.person, size: 25,color: Color(0xff3F5185), 
              ),),
              Column(
                ...
                spacing: 10,
                children: [
                  Text("Good Morning👋",style: TextStyle(fontSize: 16,fontWeight: .w400,color: Colors.grey),),
                  Text(fullName,style: TextStyle(fontSize: 20,fontWeight: .bold,color: Colors.black),)
                ],
              )
      ],
    );
...
class TaskInfoDetails extends StatelessWidget {
  const TaskInfoDetails({super.key ,required this.numOfDon,required this.numOfPending,required this.numOfTasks});
...
class TaskItem extends StatelessWidget {
  const TaskItem({super.key, required this.task, this.onTap,required this.delete});
